...
const chartUrl = 'http://localhost:4000/charts'
...
const updateForm = document.querySelector('#update-chart-form')
//Update and Delete
...
function renderForm(id, rsi, target, market, rating) {
  alert("╭∩╮(▀̿Ĺ̯▀̿ ̿)ᕗ HaHa I'll never work for you!!( ° ͜ʖ͡°)╭∩╮")
  // document.querySelector(`#chart-${id} .rsi`).innerHTML = `<input class="rsi" value="${rsi}">`
  // document.querySelector(`#chart-${id} .target`).innerHTML = `<input class="target" value="${target}">`
  // document.querySelector(`#chart-${id} .market`).innerHTML = `<input class="market" value="${market}">`
  // document.querySelector(`#chart-${id} .rating`).innerHTML = `<input class="rating" value="${rating}">`

}

//stock collection
function putChartOnPage() {
  data.forEach(chart => {
    chartCollectionDiv.innerHTML += `
    <div class="card-collection" id='chart-${chart.id}'>
    <img style="width: 70%" src="${chart.image}" class="chart-avatar">
    <h1><a class="chart-name" href="https://www.tradingview.com/symbols/${chart.name}/">${chart.name}</a></h1>
    <h4 class="rsi"> RSI: ${chart.rsi}</h4>
    <h4 class="target"> Target: ${chart.target}</h4>
    <h4 class="market"> Market Cap: ${chart.market}</h4>
    <h4 class="rating"> concensus: ${chart.rating}</h4>
    <button id="upbutt" onclick="renderForm(${chart.id}, ${chart.rsi}, ${chart.target} )">Update</button>
    <br>
    <button id="delbutt" onclick="deleteData(${chart.id})">Delete</button>
    </div>
...
// lightChart
var chart = LightweightCharts.createChart(document.querySelector('#light-charts'), {
	width: 1320,
  height: 600,
	layout: {
		backgroundColor: '#000',
...
candlestickSeries.setData([
	...
	{ time: '2019-05-25', open: 194.38, high: 196.47, low: 193.75, close: 194.08 },
	{ time: '2019-05-26', open: 196.38, high: 196.47, low: 194.75, close: 198.08 },
	{ time: '2019-05-27', open: 197.38, high: 198.47, low: 195.75, close: 196.08 },
	{ time: '2019-05-28', open: 199.38, high: 201.47, low: 196.75, close: 197.08 },
	{ time: '2019-05-29', open: 198.38, high: 200.47, low: 193.75, close: 200.08 },
	{ time: '2019-05-30', open: 199.38, high: 207.47, low: 203.75, close: 207.08 },
	{ time: '2019-05-31', open: 210.38, high: 217.47, low: 209.75, close: 215.08 },
]);
